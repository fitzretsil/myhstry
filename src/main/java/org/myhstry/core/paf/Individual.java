...
public class Individual implements Comparable<Individual>, Serializable {
	...
	/**
	 * Serial Number Generated so that we can save or transfer this data over a
	 * network or on disk.
	 */
	...
	/*
	 * each set of repeat individuals is assigned a different set number (e.g.
	 * Brigham Young repeated 5000 times (numberOfOccurances) will be set #23
	 * (repeatSetNumber) while George Washington repeated 314 times might be set
	 * number 26
	 */
	...
	public String toString() {
		if (label == null) {
			...
			label = surname + ", " + this.givenName + " " + middleName + " ("
					+ ((birth != null && birth.date != null) ? birth.date : " ") + "-"
					+ ((death != null && death.date != null) ? death.date : " ") + ")";
		}
		return label;
	}
	...
	public boolean hasMarriagePlace() {
		return (fams != null && fams.size() > 0 && fams.get(0) != null && fams.get(0).marriage != null
				&& fams.get(0).marriage.place != null && fams.get(0).marriage.place.compareTo("") != 0);
	}
	...
	public boolean isMarried() {
		if (fams != null && !fams.isEmpty()) {
			return fams.get(0).husband != null && fams.get(0).wife != null;
		}
		return false;
	}
